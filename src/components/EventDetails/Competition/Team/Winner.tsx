...
import { TeamWinner } from "@/components/events";

const Winner = ({ teamName, image, members }: TeamWinner) => {
  ...
  return (
    <>
      <div className="relative rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 group animate-fade-in-up bg-white">
        ...
        >
          <div className="absolute inset-0 bg-gradient-to-br from-yellow-400/20 via-orange-500/20 to-yellow-600/20 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10" />
          <Image
            fill
            alt={teamName}
        ...
        <div className="p-6 bg-gradient-to-br from-yellow-50 to-orange-50 border-t border-yellow-200">
          ...
          <div className="grid grid-cols-2 gap-3">
            {members.map((member, idx) => (
              <div
                className="px-3 py-2 bg-white/70 backdrop-blur-sm rounded-lg border border-yellow-200 hover:border-yellow-400 transition-colors shadow-sm hover:shadow-md"
                key={idx}
              >
